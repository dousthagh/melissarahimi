    ...
    <div class="white-box">
        @foreach (GetUserCategoriesWithLevel(auth()->user()) as $key => $value)
            @foreach ($value['categories'] as $categoryKey => $categoryValue)
                <a class="text-danger"
                    href="@if ($value['level']['order'] == 5 || $value['level']['order'] == 6) {{ route('user_level_category.master.my_student', ['userLevelCategoryParentId' => $categoryValue['user_level_category_id']]) }}
                                            @else
