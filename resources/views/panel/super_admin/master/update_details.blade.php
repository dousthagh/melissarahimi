...
@section('content')
    <div class="white-box">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <form class="form-horizontal" action="{{route('super_admin.master.files.save')}}"
                          method="post" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" value="{{$user_level_category_id}}" name="user_level_category_id"/>
    ...
    <div class="white-box">
        <table class="tablesaw table-striped table-hover table-bordered table tablesaw-columntoggle"
            ...
            <thead>
            <tr>
                <th scope="col" data-tablesaw-sortable-col="" data-tablesaw-priority="persist">نام فایل</th>
                <th scope="col" data-tablesaw-sortable-col="" data-tablesaw-sortable-default-col=""
                    data-tablesaw-priority="4" class="tablesaw-priority-4">نوع فایل
                </th>
            ...
            @foreach($files as $file)
                <tr>
                    <td class="title">{{$file->file_path}}</td>
                    <td class="tablesaw-priority-4">{{$file->file_type}}</td>
                    <td class="tablesaw-priority-2">
                        <a target="_blank" href="{{route('master.details.file.download', ['file_id'=>$file->id])}}" class="btn btn-info btn-rounded">نمایش</a>
                        <a href="{{route('super_admin.master.files.delete', ['id' => $file->id])}}" class="btn btn-danger btn-rounded">حذف</a>
                    </td>
                </tr>
            @endforeach
            </tbody>
