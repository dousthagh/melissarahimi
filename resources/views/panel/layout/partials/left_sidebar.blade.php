    ...
    <div class="sidebar-nav slimscrollsidebar">
        ...
        <ul class="nav nav-second-level">
            ...
            @foreach(GetUserCategoriesWithLevel(auth()->user()) as $key=>$value)
                <li>
                    <a href="#" class="waves-effect"><i class="mdi mdi-account-star fa-fw" data-icon="v"></i> <span
                            class="hide-menu"> {{$value['level']["title"]}} <span class="fa arrow"></span></span></a>
                    @foreach($value['categories'] as $categoryKey=>$categoryValue)
                        <ul class="nav nav-second-level">
                            <li>
                                <a href="
                                       @if($value['level']['order'] == 5 || $value['level']['order'] == 6)
                                        ...
                                        @endif
                                "><span
                                        class="hide-menu">{{$categoryValue['title']}}</span></a></li>
                        </ul>
                    @endforeach
                </li>
            @endforeach
